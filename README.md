...
- **回访识别（设备身份）**：同一手机扫码＝模拟同一来电号码（localStorage 记手机号当 caller ID），回访开场即复述上次车牌/公司/事由，全程不再问手机号。（决策 012）
- **公司名硬白名单 + 拼音模糊匹配**：仅园区名单公司可登记；音近/误听（如「陈兴物流」→「晨星物流」）靠 pypinyin 拼音相似度召回。（决策 014）
- **车牌校验数据层**：31 省份简称闭集 + 近音纠错；车牌是开集，陌生车随到随登。（决策 011）
- **成功自动挂断**：成功语「欢迎进入园区」注册为 Vapi endCallPhrases，念完自动结束通话。
- **企业微信群推送**：走腾讯接口、海外云可达稳定、免企业认证。（决策 013）
- **门卫自然语言查询**：LLM 抽结构化意图 + 参数化模板 SQL（零注入、零错 SQL）；无 LLM key 自动降级关键词规则。（决策 004）
