...
- **启动方式**：未变（仍 AWS ECS Express 固定地址，push 即上线）。**待真机验收** + 上述 Vapi 控制台两处配置。
...
## 待办（按优先级，开发优先）
0. **设备身份回访上线接线（决策 012）**：Vapi 控制台两处配置——① system prompt 粘最新版（含 `{{known_plate}}`）；② register 工具请求体加 `source_call_id={{call.id}}`。配完真机验收：同一手机首访→隔次再扫码应「接通即认出」。
