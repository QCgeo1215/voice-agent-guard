...
## 技术栈（截至 6/22，当前状态）
- 语音编排：Vapi（STT=OpenAI `gpt-4o-transcribe` `zh` / LLM=OpenAI `gpt-5` / TTS=MiniMax `speech-02-turbo` 中文音色）
- 后端：FastAPI + 数据层 SQLite（本地）/ Neon Postgres（云，`DATABASE_URL` 切换）
- 推送：云上 企业微信群机器人（决策 013）；Server酱/pushplus 本地/国内出口备选
- 公网：AWS ECS Express 固定地址（cloudflared 已退役）
- 自动挂断：成功语「欢迎进入园区」注册为 Vapi endCallPhrases
- 查询 Agent LLM：OpenAI 兼容接口（默认 DeepSeek，可留空降级关键词）
> 历史栈值（nova-3 / GPT-4.1 / Azure 等）保留在下方各日历史小节，不在此覆盖。
...
## 6/21 下午（/call 重写 + 设备身份回访识别）
两处由真机反馈驱动的开发：

- **`/call` 改用 Vapi Web SDK 自建 UI**（替代黑盒 embed widget）：解决手机端浮窗溢出/按钮不居中、连接静默退回、微信内置浏览器用不了麦克风三件事。
  - 响应式居中按钮（`min(320px,100%)`）；显式状态机 idle/connecting/active + 20s 超时与错误重试提示。
  - 检测 `MicroMessenger` → 弹横幅「右上角→在浏览器打开」并禁用按钮（连接不稳根因已确认是微信浏览器，系统 Chrome/IE 稳定）。
  - SDK 经 `esm.sh` 动态 import，加载失败有兜底。
- **设备身份「接通即识别回访」（决策 012）**：讨论澄清——回访本就用车牌做钥匙、与手机号无关；电话相对 Web 只多给「接通即识别」（caller ID 在零输入时给身份键），而 Web 拿不到设备真实号码。结论：用访客手机 **localStorage 记住上次车牌**当等价身份键，不接电话、不掉 STT 音质。
  - 前端：读 `localStorage["vg_known_plate"]`，非空时 `vapi.start` 经 `assistantOverrides.variableValues` 带 `known_plate` 并覆盖开场白（不再问已知车牌）；通话用 `call.id` 在结束时 `GET /visit/by-call/{id}` 回查车牌写回本机。新设备零回归。
  - 后端：新增只读 `GET /visit/by-call/{call_id}`（只回车牌，查不到回 null）。
  - Vapi 控制台两处配置（需手动）：system prompt 加 `{{known_plate}}` 上下文 + 第0步「本机车牌非空就接通即查」规则；register 工具请求体加 `source_call_id={{call.id}}`（顺带给 Web 通话补幂等键）。
  - 诚实边界：是「同一浏览器」身份非「同一人」；换机/清缓存优雅退回报车牌流程；首访仍需采集（与电话一致）。
- **验证**：`/call` 渲染 200、关键标记齐；`/visit/by-call` 未命中→null / 命中→车牌；`pytest` 23 绿。
- **启动方式**：未变（仍 AWS ECS Express 固定地址，push 即上线）。
- **真机验收通过（6/21 下午）**：同一手机连续两次扫码→第二次「接通即关联车牌、自动识别回访」，整条链路（call.id→后端按 source_call_id 存→前端 `/visit/by-call` 回查→localStorage→下次带回 Vapi）线上跑通。Vapi 控制台两处配置已生效。

## 6/21 晚（推送通道：云上改企业微信群机器人，决策 013）
- 真机发现：登记成功但**保安一直收不到微信**。诊断（临时 `/debug/reach` 从 ECS 出口实测）：`sctapi.ftqq.com` 解析出两个 IP，其中一个从 AWS 新加坡出口被**黑洞丢包**，而 `requests` 只连首个解析 IP、不自动切换 → **时通时不通**；同一时刻本机直连 Server酱 1.9s 正常。
- **决策 013**：云上主推送通道改 **企业微信群机器人**（腾讯 `qyapi`、海外云稳定、免企业认证），探测确认其 POST 路径 351ms 干净可达。Server酱/pushplus 降级为本地/国内出口备选。
- 改动：`notifier.py` 加 `_wecom_send`（显式判 `errcode!=0`）、`config.py` 加 `WECOM_WEBHOOK_KEY`、`cd.yml` 注入该 secret 且 `NOTIFIER_PROVIDER` 默认 `wecom`。临时探测端点已删。`pytest` 23 绿。
- **待用户操作**：建企业微信群+群机器人取 webhook key → GitHub 配 secret `WECOM_WEBHOOK_KEY` → 重跑 CD → 端到端实测（保安群收到登记卡片）。
- 启动方式：未变（AWS ECS Express 固定地址）。

## 6/21 夜 / 6/22 凌晨（WeCom 落地 + STT/LLM 大换 + 回访改手机号 + 减确认 + 公司拼音匹配 + 成功自动挂断）
> 当天改动较大，技术栈与交互都有调整；与上方早先条目冲突属正常迭代，以本节为当日最新。

### 技术栈大换（Vapi 控制台侧，值由 Vapi API 回读实测）
- **STT**：Deepgram Nova-3 → 试 Gladia Solaria（连不上，`endedReason=gladia-transcriber-failed`，且 Gladia 在 Vapi 要自带 key）→ 定为 **OpenAI `gpt-4o-transcribe`（language=zh）**。
- **LLM**：→ **OpenAI `gpt-5`**（更强，利于工具调用可靠性）。
- **TTS**：MiniMax `speech-02-turbo`，音色 `Chinese (Mandarin)_Reliable_Executive`，speed 1.2。
- 从→到：早先（6/20）为 Nova-3 + GPT-4.1 + MiniMax；本轮把 STT、LLM 都换了（TTS 仍 MiniMax）。

### WeCom 推送真正打通（决策 013 落地，commit 314d67c）
- 用户建企业微信群机器人取 webhook key → GitHub secret `WECOM_WEBHOOK_KEY` → CD 重部署。
- 复盘：先前“AI 说已通知但群里没消息”根因**不在通道**，而在工具没被调用（见下）；通道本身 ~351ms 可达。

### 设备身份回访：车牌键 → 手机号键（决策 012 修订，commit 61059d0）
- 用户澄清：手机扫码＝模拟“同一手机号来电”，应以**手机号**当设备身份键（caller ID）；回访开场即复述车牌/公司/事由，全程不再问手机号。
- 前端 localStorage 改用 `vg_known_phone`；`assistantOverrides.variableValues` 带 `known_phone/known_plate/known_company/known_reason`，开场白据此三选一（老客有历史 / 认识号无历史 / 新设备）。
- 真机验证：同机连续两次扫码 → 第二次开场即复述历史、不再问手机号。

### 减过度确认 + 工具调用可靠性（核心 badcase）
- 现象：Agent 反复确认（同学吐槽“在拖时间”）；更糟——说“我在等系统返回登记结果”却**从不真正调用 `register_visitor`**，最后 `[object Object]` 挂断、群里收不到。
- 根因：register 工具描述含“确认后再调”的冲突指令 + 模型用话术代替动作。
- 修复：①（Vapi API）改 register 工具描述为“凑齐即调、无需复述”；② prompt 立死规矩“通知保安唯一方式＝真调工具，绝不用嘴替代”；③ 校验保留基本项（手机号 11 位 / 公司白名单 / 车牌规则），删过拟合的反复确认。
- 结果：工具真被调用、企业微信群收到登记卡片。

### 公司名硬拦截 + 拼音模糊匹配（决策 014，commit db08386）
- 保留硬白名单（仅名单内可登记），加 `pypinyin` 拼音相似度，提升音近/误听召回（如“陈兴物流”→“晨星物流”）；真未知仍硬拒。
- 改 `company_registry.py`（字符级与拼音级相似度取最大值排序）、`requirements.txt` 加 `pypinyin`、加同音字测试用例。

### 成功登记后自动挂断（本次会话，commit e849bd8）
- 成功语以「欢迎进入园区」收尾，注册为 Vapi `endCallPhrases`，念完自动挂断；去掉矛盾的“您稍等”，并消除“先说通知保安、后说登记好”的重复。
- 一度加 `endCall` 工具想让“转人工”也优雅挂断 → 用户判定 demo 无真人工、属画蛇添足 → 回退（`endCallFunctionEnabled=False`），仅留成功路径自动挂。

### 验证
- 后端 `pytest` **29 绿**。
- 线上探活：`/health` `/guard` `/guard/query` `/visitors` 全 **200**；查询四类（计数/明细/按公司/按事由）+ 日期范围正常。门卫后台日期按**服务器 UTC** 计（非 bug）。
- DB 现 6 条，含 2 条诊断探针（“…请忽略”），demo 前可清。

### 坑 / 教训
- Gladia 在 Vapi 需自带 API key（非 bundled），本场景报 transcriber-failed，弃用。
- `endCallPhrases` 触发词必须放成功语**最末尾**，否则尾字可能被截。
- 排推送问题先确认工具是否**真触发**（拉 call trace）——“AI 嘴上说已通知 ≠ 真调用”。
- 改 `vapi/system_prompt.md` 后仍须手动粘回 Vapi（仓库不自动同步，老坑）。

### 启动 / 前提变化
- 启动方式未变（AWS ECS Express 固定地址，push 即上线）。
- 新前提：Vapi 侧 STT/LLM/TTS 已换（见上）；后端成功语收尾必须与 `endCallPhrases`「欢迎进入园区」一致才会自动挂断。

### 交接（6/22 收尾为主）
- demo 视频已录。
- 剩：架构图/流程图、README 压一页、决策记录补全（013/014 已建）、（如仍需）25s 端到端实测文档、答辩材料。
- 收尾（6/22 凌晨）：已刷新顶部「技术栈」块与「待办 0」为最新；Vapi 默认开场白由英文「hello…」改回中文「您好，请问车牌号多少？…」（`system_prompt.md` 同步）；`README.md` 整体更新到当前架构（云部署/技术栈/接口/环境变量）。

## 关键文件（相对 6/16 有更新）
- `backend/main.py`：字段校验 + 回访识别 + `lookup_visitor` 端点 + `/call`（Vapi Web SDK 自建 UI）+ 只读 `/visit/by-call/{id}`（设备身份，决策 012）
...
- `docs/decisions/001~012`：十二条决策记录（003 被 007 升级，005 被 009 修正，008 手机入口，010 CI/CD+AWS，011 车牌校验数据层，012 设备身份回访识别）
...
## 待办（按优先级，开发优先）
0. ~~**推送通道接线（决策 013）**~~ ✅ **已完成（6/21 夜）**：WeCom 群机器人 webhook + GitHub secret `WECOM_WEBHOOK_KEY` 配好、CD 重部署、端到端实测保安群收到登记卡片。
1. ~~设备身份回访上线接线（决策 012）~~ ✅ **真机验收通过（6/21）**：Vapi 两处配置已生效，同一手机连续两次扫码自动关联车牌、接通即识别。
1. **必须项：25s 端到端实测 + 实战测试迭代**（题目硬指标，需形成文档证据 `docs/test_runs.md`）：正常新访客 / 回访 / 缺字段 / 错手机号 / 改口 / 未知公司 / 车牌纠错 / 设备身份回访。
...
- **云→国内推送被 IP 黑洞**（决策 013）：Server酱/pushplus 等国内小服务从 AWS 海外出口可能连接超时（多 IP 中部分被丢包，`requests` 不自动切 IP）；云上用企业微信群机器人（腾讯接口）规避。换通道只改 `NOTIFIER_PROVIDER` + 对应 key。
...
1. **设备身份回访接线 + 验收（决策 012）**：Vapi 控制台配 `{{known_plate}}` 与 `source_call_id={{call.id}}`；真机「同机隔次扫码 → 接通即识别」。
2. **25s 端到端掐表 + 实测迭代**：用固定地址 + 手机真机跑多场景，结果记 `docs/test_runs.md`（模板待建）。
3. **车牌纠错 / 回访体验真机验收**：6/20 改的 `plate_registry` 与回访话术需真机过一遍。
